...
| R2 | サービス層 + DI コンテナ（NON-BREAKING）: `internal/service/` に Host/Service/Project/DNS… を作成しハンドラから enrichment ループ・tag/responsável 同期・alert/usage 計算・cross-entity orchestration を移設（silent error swallow も修正）。`cmd/app.go` で `db→Encryptor→repos→services→handlers` を一括 wiring。`secretHandlers.register(mux,wrap)` の自己登録パターンを全ハンドラ群へ一般化。ハンドラを thin 化 | ハンドラテストを service mock 化; `go test ./...` green; golden response 比較で behavior-identical | R1 | cc:WIP（DNS ✅[d8a0cb0] / Project ✅[384130b] / Service ✅[5cb0bf2] / **Host ✅** が full vertical（repo+service+thin handler+DI）完了。Host: repo[288428d] → HostService read paths（List/Get enrichment+alert 計算）[e07b8b0] → write paths（create/update/delete・vault dual-write・SSH-key link・cascade）[420e529]。handler 残置は decode/validation・preferred-auth normalize・grafana fire-and-forget・getPassword/resolveHostKeyPEM（transport/integration、一部 R4）。**4 core aggregate 完了**。残 R2: `cmd/app.go` container（30-struct soup 解体）、`secretHandlers.register` 自己登録パターンの全ハンドラ一般化） |
