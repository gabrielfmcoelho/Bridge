    ...
    <div className="fixed inset-0 z-[60] flex items-start justify-center pt-[15vh] bg-black/60" onClick={onClose}>
      <div className="w-full max-w-xl bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-[var(--radius-lg)] shadow-[var(--shadow-lg)] overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-2 px-3 py-2 border-b border-[var(--border-subtle)]">
          ...
          />
          <kbd className="text-3xs px-1 py-0.5 rounded bg-[var(--bg-elevated)] text-[var(--text-faint)]">esc</kbd>
        </div>
            ...
            results.map((n, i) => {
              ...
              return (
                ...
                >
                  ...
                  {n.layer && <span className="text-3xs text-[var(--text-muted)] shrink-0">{n.layer}</span>}
                  {n.namespace && <span className="text-3xs text-[var(--text-muted)] shrink-0">{n.namespace}</span>}
                </button>
              );
            })
          )}
