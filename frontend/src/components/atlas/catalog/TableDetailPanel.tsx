    ...
    <div className="flex flex-col gap-5">
      ...
      <header className="flex flex-col gap-2 -mt-1">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <span className="text-2xs text-[var(--text-faint)] ml-1">
            {rec.namespace}
          </span>
        </div>
...
function NodeChip({ node }: { node: LineageNode }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 text-xs font-mono px-2 py-1 rounded-[var(--radius-sm)] border ${color}`}>
      <span className="text-3xs opacity-80">{node.type.replace("_", " ")}</span>
...
function ColumnRow({
  ...
  return (
    ...
    >
      ...
      {inferred && (
        <span className="text-3xs text-[var(--text-faint)]">{t("atlas.catalog.column.inferred")}</span>
      )}
