export default function PillButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      className={`inline-flex items-center px-3 py-1.5 text-xs rounded-[var(--radius-md)] border transition-all duration-150 font-medium whitespace-nowrap active:scale-[0.97] ${
        active
