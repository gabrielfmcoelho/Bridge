        ...
        return (
          <div key={stepNum} className="flex items-center gap-2 flex-1 min-w-0">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold shrink-0 transition-colors ${
              current === stepNum ? "bg-[var(--accent)] text-white" : current > stepNum ? "bg-[var(--success)]/20 text-[var(--success)]" : "bg-[var(--bg-elevated)] text-[var(--text-faint)]"
            }`}>
            ...
            {!isLast && <div className={`flex-1 h-px min-w-4 ${current > stepNum ? "bg-[var(--success)]/40" : "bg-[var(--border-subtle)]"}`} />}
          </div>
        );
      })}
      {/* Step counter */}
