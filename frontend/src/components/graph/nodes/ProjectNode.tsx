...
export default function ProjectNode({ data }: NodeProps) {
  ...
  return (
    <div className="rounded-[10px] min-w-[180px] shadow-lg cursor-pointer transition duration-200 hover:shadow-xl overflow-hidden" style={{ background: "var(--bg-surface)", border: "1px solid var(--border-default)" }}>
      <Handle type="target" position={Position.Top} className="!bg-[var(--warning)] !w-2 !h-2" />
      <div className="h-1 bg-gradient-to-r from-[var(--warning)] to-[var(--warning)]" />
      <div className="px-3 py-2">
        <div className="flex items-center gap-2">
          <svg className="w-3.5 h-3.5 text-[var(--warning)] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
