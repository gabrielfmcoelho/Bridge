...
export default function AlertBadge({ alerts }: { alerts?: HostAlert[] }) {
  ...
  return (
    <Tooltip
        ...
        <div className="space-y-1.5">
          {alerts.map((a, i) => (
            <div key={i} className="flex items-start gap-2">
              ...
              <div>
                <span className={`text-2xs font-semibold uppercase ${ALERT_TEXT_COLOR[a.level]}`}>{a.level}</span>
      ...
      side="bottom"
    >
      <span className={`inline-flex items-center gap-0.5 text-2xs font-semibold cursor-help ${ALERT_TEXT_COLOR[worstLevel]}`}>
